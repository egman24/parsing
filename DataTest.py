...
def get_doc_number(): return str(uuid.uuid4().int)


def get_kind(): return random.choice(['A1', 'A'])


def get_country(): return random.choice(['EP', 'JP', 'US'])


def get_date(): return '20170201'


def get_assignee(): return random.choice(
    ['A Corp.', 'B Inc.', 'C Corp.', 'D Inc.', 'F Corp.', 'G Inc.'])
...
def create_document():
    ...
    document = DataImport.create_document('Test', '?', dnum, doc_number, kind, get_date(
    ), 'n', country, '', '', get_date(), get_date(), get_date())
...
def create_assignee(assigned_document):
    ...
    ), '12345', '?', '?', get_date(), get_date(), get_date(), get_date())
...
def create_citation(citer_document, cited_document):
    ...
    DataImport.add_citation(citer_document['doc_number'], cited_document['doc_number'], citer_document['country'], citer_document['kind'],
                            citer_document['datepublished'], citer_document['filedate'], citer_document['issuedate'], citer_document['prioritydate'])
...
def create_citations_random(focus_document, related_count=10):
    ...
    [create_morph_relation(focus_document, create_assignee(
        citation_document)) for citation_document in related_documents]
    print('starting pass 2...')
    [create_morph_relation(focus_document, citation_document) for citation_document in random.sample(
        related_documents, math.floor(number_of_documents / 2))]
    print('starting pass 3...')
    [create_morph_relation(focus_document, citation_document) for citation_document in random.sample(
        related_documents, math.floor(number_of_documents / 4))]
    print('starting pass 4...')
    [create_morph_relation(focus_document, citation_document) for citation_document in random.sample(
        related_documents, math.floor(number_of_documents / 8))]
