...
def classifications(bibliographic):
    ipcr = bibliographic.find(
        '{http://www.epo.org/exchange}classifications-ipcr')
...
def buildeachcitation(citations, data):
    ...
    citation = {'dnum': dnum, 'dnum-type': dnumtype, 'doc-id': docid, 'country': country,
                'doc-number': docnumber, 'kind': kind, 'name': name, 'date': date}
...
def citations(bibliographic):
    citations = []
    referencescited = bibliographic.findall(
        '{http://www.epo.org/exchange}references-cited')
    citation = fmap(lambda x: x.findall(
        '{http://www.epo.org/exchange}citation'), referencescited)
    patcit = fmap(lambda x: fmap(lambda y: y.findall('patcit'), x), citation)
    documentid = fmap(lambda x: fmap(lambda y: fmap(lambda z: fmap(lambda doc: {'doc': doc, 'dnum': z.attrib.get(
        'dnum', ''), 'dnum-type': z.attrib.get('dnum-type', '')}, z.findall('document-id')), y), x), patcit)
    fmap(lambda x: fmap(lambda y: fmap(lambda z: fmap(
        buildcitations(citations), z), y), x), documentid)
    return citations
...
def familymembers(family):
    members = maybe(family).findall(
        '{http://www.epo.org/exchange}family-member').or_else([])
...
def to_database(metadata):
    ...
    title = metadata['title'][0].get('title', '') if len(
        metadata['title']) > 0 else ''
    ...
    DataImport.create_document('Bibliographic', '?', '', doc_number,
                               kind, date_publ, status, country, title, '', '', '', '')

    for citation in citations:
        DataImport.add_citation(doc_number, citation.get(
    ...
    for classification in classifications:
        DataImport.add_classification(doc_number, classification.get('section', ''), classification.get('class', ''), classification.get(
            'subclass', ''), classification.get('main-group', ''), classification.get('subgroup', ''), date_publ, '', '', '')

    for assignee in assignees:
        DataImport.add_assignee(doc_number, maybe(assignee)['docdb']['name'].or_else(
            ''), '', '', '', date_publ, '', '', '')

    for member in family_members:
        application = member['application-references']['docdb']
        DataImport.add_family_member(
            'FamilyApplication', family_id, application['doc-number'], application['country'], application['kind'], application['is-representative'])
        publication = member['publication-references']['docdb']
        for sequence, document in publication.items():
            DataImport.add_family_member(
                'FamilyPublication', family_id, document['doc-number'], application['country'], application['kind'])
